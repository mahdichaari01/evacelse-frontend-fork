...
export const QuestionSkeleton = () => (
  <div className="h-full w-full overflow-hidden ui-part">
    <div className="flex w-full h-full justify-between items-center flex-col gap-5 p-1 md:p-[2.6rem]">
      ...
      <div className="flex flex-col w-full justify-evenly h-full gap-3 overflow-hidden">
        ...
        <ScrollableBox className="w-full rounded h-full">
          <ul className="flex w-full flex-col gap-[.625rem]">
            {Array.from({ length: 3 }).map((_, index) => (
              <li
                key={index}
                className="flex flex-row m-0.5 justify-start gap-[.875rem] "
              >
                <div
                  className={`flex shadow-sm w-12 place-content-center place-items-center rounded p-3 text-3xl font-bold bg-clears-tertiary animate-pulse text-darks-low `}
                >
                  <span>{String.fromCharCode(index + "A".charCodeAt(0))}</span>
                </div>
                <div
                  className={`flex w-full shadow-sm items-center justify-start rounded p-3 bg-clears-tertiary animate-pulse text-darks-low `}
                >
                  <span>{}</span>
                </div>
              </li>
